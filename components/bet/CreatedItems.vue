...
import type { UserBetModel, BetModel } from "~/types/api-bet.type";
...
const betStore = useUserBetStore();

const props = defineProps<StateType>();

const emit = defineEmits<{
  "bet-click": [bet: UserBetModel];
}>();
...
const getBetStatus = (bet: UserBetModel) => {
  if (bet.win == true) {
...
const getPredictionText = (prediction: string, match: BetModel) => {
  switch (prediction) {
    case "home":
...
const getFinalResult = (bet: UserBetModel) => {
  if (bet.win == true) {
    return {
...
function handleBetClick(bet: UserBetModel) {
  emit("bet-click", bet);
}

async function deleteBetClick(bet: UserBetModel) {
