...
import type { BetCreationData } from "~/types/api-bet.type";

// type StateType = {
//   bets: BetCreationData[];
//   isLoading: boolean;
// };
...
const emit = defineEmits<{
  "bet-click": [bet: BetCreationData];
  "edit-bet": [bet: BetCreationData];
  "toggle-status": [bet: BetCreationData];
  "end-bet": [bet: BetCreationData];
  "delete-bet": [bet: BetCreationData];
}>();

// Obtenir le statut du pari
const getBetStatus = (bet: BetCreationData) => {
  if (bet.isEnded) {
...
function handleBetClick(bet: BetCreationData) {
  emit("bet-click", bet);
}

function handleEdit(bet: BetCreationData, event: Event) {
  event.stopPropagation();
  emit("edit-bet", bet);
}

function handleToggleStatus(bet: BetCreationData, event: Event) {
  event.stopPropagation();
  emit("toggle-status", bet);
}

function handleEndBet(bet: BetCreationData, event: Event) {
  event.stopPropagation();
  emit("end-bet", bet);
}

function handleDelete(bet: BetCreationData, event: Event) {
  event.stopPropagation();
  emit("delete-bet", bet);
}
</script>
